  ...
  <div v-else>
    <form id="remote-search" :class="['ui', {loading: isLoading}, 'form']" @submit.stop.prevent="submit">
      ...
      <div class="ui required field">
        ...
        <p v-if="type === 'rss'">
          <translate translate-context="Content/Fetch/Paragraph">Use this form to subscribe to a RSS feed from its URL.</translate>
        </p>
        <p v-else-if="type === 'artists'">
          <translate translate-context="Content/Fetch/Paragraph">Use this form to subscribre to an channel hosted somewhere else in the Fediverse.</translate>
        </p>
  ...
  computed: {
    labels() {
      ...
      if (this.type === "rss") {
        ...
        title = this.$pgettext('Head/Fetch/Title', "Subscribe to a podcast hosted in the Fediverse")
