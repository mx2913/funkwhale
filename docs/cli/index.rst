...
However, we've compiled :doc:`a list of example uses of the CLI <examples>` with advices and explanations, if you want to check it out ;)

Getting help
------------

The most basic way to get help is to run ``funkwhale --help``. It will list you available commands, namespaces and arguments that are common to all commands.
...
2. Explicitely pass a token to the command via the ``-t`` flag or the ``FUNKWHALE_TOKEN`` environment variable

If you use ``funkwhale login``, you can delete the local token with ``funkwhale logout``.

You can check that you are fully authentified by running ``funkwhale users me``. It will display information regarding your user profile.
...
All commands that list results, such as ``funkwhale albums ls`` or ``funkwhale tracks ls`` share a similar behaviour and set of arguments.
...
.. note::
    ...
    Ordering in descending order is supported by prefixing the field name by ``-``, e.g: ``-o -creation_date``
...
You can of course combine those flags::

    # retrive 3 pages of 12 albums, starting on the 4th page
...
Note that this is also technically equivalent to applying both ``--no-headers``, ``--format plain`` and ``--column ID`` flags.
...
By default, the ``rm`` command will ask for confirmation, but you can disable this behaviour by providing the ``--no-input`` flag.
